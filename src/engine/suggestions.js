...
// ── Mood-biased chord scoring ─────────────────────────────────────────────────
// mood: { valence: -1..1, tension: -1..1 }
// valence > 0 → prefer major/bright chords; < 0 → prefer minor/dark chords
// tension > 0 → prefer complex extensions; < 0 → prefer simple triads
...
export function computeSuggestionsWithMood(progression, allChords, mood = {}, topN = 40) {
  ...
  return new Map(
    allChords
      .map(c => [c.name, Math.min(1, Math.max(0, scoreChord(c, last, root, scale) + moodBias(c, mood)))])
      .filter(([, s]) => s > 0.05)
      .sort((a, b) => b[1] - a[1])
      .slice(0, topN)
  )
